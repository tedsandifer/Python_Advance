...
dependency   -arNotice, arUtil, tank
how to       -
todo         -
...
class ArLoad(ArUtil):
    ...
    def __init__(self):
        ...
        path_ui = "/".join([os.path.dirname(__file__), 
                            "ui",  
                            TITLE + ".ui"]
                            )
        ...
        self.software_format = {y:x.upper() for x,y in
                                self.data['software']['EXTENSION'].items()}
    ...
    def press_btn_accept(self):
        if not os.path.exists(self.load_file):
            self.set_status('FAILED LOADING : Path doesn\'t exists: {}'
                            .format(self.load_file), 
                            msg_type=3
                            )
            return False
